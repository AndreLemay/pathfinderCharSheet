...
interface OwnProps {
	...
	attackComponent: React.FunctionComponent<AttackProps>
}
...
class IndividualAttackContainer extends React.Component<
	IndividualAttackContainerProps
> {
	edit = () => {
		this.props.openAttackModal(
			(bundle: AttackInfoBundle) => {
...
function mapStateToProps(
	...
	return {
		...
		toHitBonus:
			getMiscAttackBonus(state) + getAbilityBonus(attack.toHitBonusAbility)
	}
}
...
function mapDispatchToProps(dispatch, props: OwnProps): DispatchProps {
	return {
		edit: bundle =>
			dispatch(editAttack(props.attackUuid, props.equipUuid, bundle)),
