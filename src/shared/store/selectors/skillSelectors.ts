import { AbilityType, SkillName, BonusType } from '../../api/enums'
...
const getClassSkill = (state: CharacterSheetState, props: OwnProps) =>
	state.skills[props.skillOrd].isClassSkill
const getRanks = (state: CharacterSheetState, props: OwnProps) =>
	state.skills[props.skillOrd].ranks
const getMiscBonus = (state: CharacterSheetState, props: OwnProps) =>
	getBonusTotal(state, {
		'includedBonuses': [
...
const getArmourPenalty = (state: CharacterSheetState, props: OwnProps) =>
	SkillName.values[props.skillOrd].checkPenaltyApplies
		? state.armour.checkPenalty + state.shield.checkPenalty
		: 0

const calcSkillBonus = (
	abilityBonus: number,
	ranks: number,
	miscBonus: number,
	...
	return (
		...
		miscBonus +
		armourPenalty
	) // expecting this be be negative, so we want to add
}

export const makeGetSkillBonus = () => {
	return createSelector(
		[getAbilityBonus, getRanks, getMiscBonus, getArmourPenalty, getClassSkill],
		calcSkillBonus
	)
}
